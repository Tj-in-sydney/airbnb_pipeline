...
def import_load_airbnb_func(**kwargs):
    ...
    # Check if the file exists (chronological order)
    filelist = sorted([k for k in os.listdir(AIRBNB) if k.endswith('.csv')])
    if not filelist:
        logging.info("No CSV files found in the AIRBNB directory.")
        return None
    ...
    for fname in filelist:
        ...
        for chunk in pd.read_csv(os.path.join(AIRBNB, fname), chunksize=1000):
            all_data.append(chunk)  # Append each chunk to the list
        
        df = pd.concat(all_data, ignore_index=True)

        if len(df) > 0:
            # Define batch size for PostgreSQL inserts
            batch_size = 10000  
            num_batches = math.ceil(len(df) / batch_size) 

            # Loop through each batch and insert into PostgreSQL
            for i in range(num_batches):
                # Slice the DataFrame to get the current batch
                batch_df = df[i * batch_size:(i + 1) * batch_size]
                
                # Prepare data for insertion
                values = batch_df[col_names].to_dict('split')['data']
                ...
                # Execute insert with batch data
                execute_values(cursor, insert_sql, values, page_size=batch_size)
                conn_ps.commit()
                
                logging.info(f"Batch {i + 1} of {num_batches} inserted successfully for {fname}.")

            # Move processed file to the archive folder
            archive_folder = os.path.join(AIRBNB, 'archive')
            if not os.path.exists(archive_folder):
                os.makedirs(archive_folder)
            shutil.move(os.path.join(AIRBNB, fname), os.path.join(archive_folder, fname))
...
# Task Dependencies
import_load_airbnb_task >> import_load_lga_g01_task >> import_load_lga_g02_task >> import_load_lga_code_task >> import_load_lga_suburb_task >> trigger_dbt_job_task
